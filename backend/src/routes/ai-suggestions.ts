...
import { activities, suggestions } from "../data/store";
...
Strictly return the response as a JSON array with the exact keys: title, description, category (must be 'email', 'storage', or 'ai'), savingsINR (number), and savingsCO2 (number).
...
${JSON.stringify(recentActivities, null, 2)}
    ...
    suggestions.unshift(...newSuggestions);
